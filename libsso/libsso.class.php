...
class SSO {
    ...
    public static function getUserInfo($id = null) {
        ...
        $ucRow = $ssoRow = null;
        $ucResult = self::$ucdb->query("SELECT `username`, `email` FROM `{$GLOBALS['__CONFIG']['uc']['dbpre']}members` WHERE `uid` = {$id}");
    ...
    public static function getUserByOpenid($openid) {
        ...
        if (!$info) {
            ...
            $user = self::$ucdb->query("SELECT `username` FROM `{$GLOBALS['__CONFIG']['uc']['dbpre']}members` WHERE `uid` = {$info['auth_id']}");
